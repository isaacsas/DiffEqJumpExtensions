mutable struct Direct2JumpAggregation{T,F1,F2} <: AbstractJumpAggregator
  next_jump::T
  end_time::T
  cur_rates::Vector{T}
  sum_rate::T
...
@inline function (p::Direct2JumpAggregation)(u,t,integrator) # condition
  p.next_jump==t
end

function (p::Direct2JumpAggregation)(integrator) # affect!
  rng_val = rand()
...
function (p::Direct2JumpAggregation)(dj,u,t,integrator) # initialize
...
@inline function aggregate(aggregator::Direct2,u,p,t,end_time,constant_jumps,save_positions)
  ...
  Direct2JumpAggregation(next_jump,end_time,cur_rates,
    sum_rate,rates,affects!,save_positions)
end

DiscreteCallback(c::Direct2JumpAggregation) = DiscreteCallback(c,c,initialize=c,save_positions=c.save_positions)
