struct JumpProblem{P,A,C,J<:Union{Void,AbstractJumpAggregator},J2,J3,J4} <: AbstractJumpProblem{P,J}
  prob::P
  aggregator::A
...
function JumpProblem(prob, aggregator::AbstractAggregatorAlgorithm, jumps::JumpSet;
  ...
  if (typeof(jumps.constant_jumps) <: Tuple{}) && (jumps.massaction_jump == nothing)
    ...
    disc = aggregate(aggregator,u,prob.p,t,end_time,jumps.constant_jumps,jumps.massaction_jump,save_positions)    
...
function Base.show(io::IO, A::JumpProblem)
  ...
  if (A.massaction_jump != nothing) && (length(A.massaction_jump.scaled_rates) > 0)
