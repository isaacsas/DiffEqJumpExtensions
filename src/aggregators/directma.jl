...
function (p::DirectMAJumpAggregation)(integrator) # affect!
    ...
    p.sum_rate,ttnj = time_to_next_jump(integrator.u,integrator.p,integrator.t,p.aggregator,p.cur_rates)
...
function (p::DirectMAJumpAggregation)(dj,u,t,integrator) # initialize
    sum_rate,next_jump = time_to_next_jump(u,integrator.p,t,p.aggregator,p.cur_rates)
    p.sum_rate         = sum_rate
...
function time_to_next_jump(u,p,t,aggregator::DirectMA,cur_rates)
    @inbounds cur_rates[1] = evalrxrate(u, 1, aggregator.scaled_rates, aggregator.reactant_stoch)
...
@inline function aggregate(aggregator::DirectMA,u,p,t,end_time,constant_jumps,save_positions)
    ...
    sum_rate,next_jump = time_to_next_jump(u,p,t,aggregator,cur_rates)
